...
let newContent = fileContent
  .replace(/const ajv = new Ajv\(\{ strict: false \}\);/g, "const ajv = new Ajv({strict: false, allErrors: true});")
  .replace(/const ajv = new Ajv\(\{ strict: true \}\);/g, "const ajv = new Ajv({strict: true, allErrors: true});");
// delete the line ajv.compile(jsonSchema);
newContent = newContent.replace(/^.*ajv\.compile\(jsonSchema\);\s*$/gm, "");
